...
export class ListRestaurantService {
  ...
  async execute(): Promise<RestaurantInterface[]> {
    ...
    const restaurantRatings = await Promise.all(
      restaurants.map(async (restaurant: RestaurantInterface) => {
        const restaurantRatings =
          await this.restaurantRatingRepository.getByRestaurantId(
            restaurant.id,
          );
        const averageRating =
          restaurantRatings.reduce((acc, rating) => acc + rating.rating, 0) /
    ...
    const restaurantsWithAverageRating = await Promise.all(
      restaurants.map(
        async (restaurant: RestaurantInterface, index: number) => {
          ...
          const dishes = await this.dishRepository.listByRestaurant(
            restaurant.id,
          );
          const minPrice =
            dishes.length > 0
