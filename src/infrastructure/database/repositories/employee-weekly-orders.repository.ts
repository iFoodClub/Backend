...
import {
  DayOfWeek,
  EmployeeWeeklyOrdersEntityInterface,
} from 'src/domain/repositories/employee-weekly-orders.repository.interface';
...
export class EmployeeWeeklyOrdersRepository {
  ...
  async create(
    employeeWeeklyOrders: Omit<EmployeeWeeklyOrdersEntityInterface, 'id'>,
  ...
  async update(
    id: number,
    employeeWeeklyOrders: Partial<
      Omit<EmployeeWeeklyOrdersEntityInterface, 'id'>
    >,
  ): Promise<EmployeeWeeklyOrdersEntityInterface> {
    const employeeWeeklyOrder =
      await this.employeeWeeklyOrdersEntity.findByPk(id);
